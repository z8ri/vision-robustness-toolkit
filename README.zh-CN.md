...
**一个工业视觉鲁棒性工具箱**：物理退化协议、样本级自适应小波域注意力模块、课程式自适应数据增强、按切片分解的鲁棒性评测、校准后的选择性预测、ONNX 部署验证，以及把这一切串起来的真实 ConvNeXt-Tiny + S-WFA 训练管线。198 个测试全部通过，跑通任何一个都不需要 GPU 或私有数据。
...
**代码从哪来。** 退化协议、S-WFA、A-PhysDeg、Robustness Cube、校准与 ONNX 这些组件，是在一个私有研究代码库里（2026 年 2–6 月）为一篇在投论文开发的。那个代码库和机密数据集绑在一起，无法公开。本仓库是从中抽取出来的、去掉数据的公开版本，于 2026-09-16 整理完成，并补上了测试和合成数据路径。这里的 commit 历史反映的是整理过程，不是原始开发的时间线。

## 实际结果

以下数字来自上面提到的私有研究代码库——在机密数据集上的 69 组 GPU 实验（5-seed 重复、跨 backbone、跨数据集），在本公开仓库存在之前就已跑完。本仓库与其共享同一套组件实现，但不含任何数据，所以**这些数字无法在本仓库复现**；列在这里是为了说明这些组件在它们本来的场景下能做到什么。Baseline 是既不带 PhysDeg 也不带 S-WFA 的 ConvNeXt-Tiny。
...
- 5 seed 取平均；跨 backbone、跨数据集结论一致，不是单次幸运结果。
